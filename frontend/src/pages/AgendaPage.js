...
function CapacityBadges({ citas }) {
  const { servicios, sesionCortesia } = useTenant();
  // "Sesión de cortesía" has no servicios row of its own — it shares
  // Pilates' capacity (same physical class), matching core/servicios.py.
  const cap = Object.fromEntries(servicios.map((s) => [s.nombre, s.capacidad]));
  if (sesionCortesia?.habilitada) {
    const pilates = servicios.find((s) => s.nombre === 'Pilates');
    if (pilates) cap['Sesión de cortesía'] = pilates.capacidad;
  }
...
export default function AgendaPage() {
  ...
  const slots = useMemo(() => buildSlots(horario), [horario]);
